...
### P-BR-004：接手 MediaCrawler 前先读取飞书同步配置

**场景：** 新团队首次接手 MediaCrawler，评估飞书链路时
**模式：**
1. 优先读取 `feishu_sync/config.py` 与 `.env` 中飞书变量说明
2. 明确 `FEISHU_TABLE_ID` 与 `TABLE1_ID` 的历史双命名关系
3. 在进入实现前确认目标表结构与字段映射是否一致
4. 若变量不全，先标记阻塞并提示补齐，不进入半配置状态开发
**验证：** MediaCrawler 接手会议与多轮迭代复盘（2026-02-26 起）
**注意：** 不读取飞书配置会导致流程理解偏差，尤其在多表同步场景
