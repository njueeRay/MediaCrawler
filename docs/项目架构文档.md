...
| 微博 | `wb` | 微博搜索、详情、博主 |
| 百度贴吧 | `tieba` | 帖子搜索、详情 |
| 知乎 | `zhihu` | 问答搜索、详情、答主 || 微信公众号 | `wechat` | 文章搜索、详情、创作者（纯 API，无浏览器） |
### 1.3 核心功能特性

- **多平台支持**：统一的爬虫接口，支持 8 大主流平台（含微信公众号）
- **多种登录方式**：二维码、手机号、Cookie、Auth-Key 多种登录方式
...
flowchart TB
    ...
    subgraph Core["核心爬虫层"]
        ...
        subgraph Platforms["平台实现"]
            ...
            tieba["TieBaCrawler"]
            zhihu["ZhihuCrawler"]
            wechat["WeChatCrawler<br/>(纯API, 无浏览器)"]
        end
    end
    ...
    subgraph Client["API客户端层"]
        ...
        wechatClient["WeChatClient<br/>(httpx→外部API)"]
    end
