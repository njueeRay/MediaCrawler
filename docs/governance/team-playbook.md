...
当用户输入存在多种合理解读时，团队必须执行以下流程：

1. Brain 先输出 2-3 个候选方案（每个方案包含适用场景、成本、风险）。
2. 明确标注推荐方案及推荐理由。
3. 等待用户选择后再进入实施，不允许在未确认前直接落代码。
4. 若用户表达片面，PM/Dev 需补充 1-3 个“可能的真实目标扩展点”，帮助用户完成目标澄清。

> 目标：避免“按字面执行但偏离真实意图”，提升一次沟通命中率。
